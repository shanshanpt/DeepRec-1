...
LOG(INFO) << "=======================> InitializeOptimizers:MemoryOptimizer:000";
      optimizers->push_back(
          // Use the default target node name prefix "gradients/"
          MakeUnique<MemoryOptimizer>(cfg_.memory_optimization()));
    } else {
LOG(INFO) << "=======================> InitializeOptimizers:MemoryOptimizer:111";
...
LOG(INFO) << "=======================> InitializeOptimizers:AutoParallel";
    optimizers->push_back(
        MakeUnique<AutoParallel>(cfg_.auto_parallel().num_replicas()));
  }
  if (cfg_.scoped_allocator_optimization()) {
LOG(INFO) << "=======================> InitializeOptimizers:ScopedAllocatorOptimizer";
