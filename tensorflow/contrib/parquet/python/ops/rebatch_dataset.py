...
from tensorflow.contrib.parquet.python.ops.rebatch import input_fields

class RebatchDataset(dataset_ops.DatasetV2):
  ...
  def __init__(
    ...
    self._impl = gen_dataset_ops.rebatch_tabular_dataset(
    ...
    super(RebatchDataset, self).__init__(self._impl)
...
def rebatch(
  ...
  Args:
    batch_size: Maxium number of samples in an output batch.
    min_batch_size: (O
    tional.) Minimum number of samples in a non-final
      batch. Same to `batch_size` by default.
    fields: (Optional.) List of DataFrame fields. Fetched from `input_dataset`
      by default.
    drop_remainder: (Optional.) If True, smaller final batch is dropped.
